...
// ✅ FIXED: path corrected to authMiddleware.js + named exports now exist
...
router.patch("/:id/pay-advance", protect, restrictTo("buyer"), async (req, res) => {
  try {
    ...
    if (!canTransition(order.status, "advance_paid")) {
      return res.status(400).json({
        success: false,
        message: `Cannot pay advance from '${order.status}' status.`,
      });
    }
    ...
    order.payment = {
      advancePaid: true,
      advancePaymentId: paymentId || "manual",
...
// ─── GET /api/orders/artist/queue — artist's incoming orders ─────────────
router.get("/artist/queue", protect, restrictTo("artist"), async (req, res) => {
  try {
...
// ─── PATCH /api/orders/:id/respond — artist accepts or rejects ───────────
router.patch("/:id/respond", protect, restrictTo("artist"), async (req, res) => {
  try {
    const { action, reason } = req.body;
    if (!action) {
      return res.status(400).json({ success: false, message: "action is required (accept | reject)." });
    }
    ...
    if (order.status !== "waiting") {
      return res.status(400).json({
        success: false,
        message: `Order must be in 'waiting' status to respond. Current: '${order.status}'.`,
      });
    }
